from http import HTTPStatus
from typing import Any, cast
from urllib.parse import quote

import httpx

from ...client import AuthenticatedClient, Client
from ...types import Response, UNSET
from ... import errors

from ...models.registration_status import RegistrationStatus
from typing import cast



def _get_kwargs(
    
) -> dict[str, Any]:
    

    

    

    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api/auth/registration-status",
    }


    return _kwargs



def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> RegistrationStatus | None:
    if response.status_code == 200:
        response_200 = RegistrationStatus.from_dict(response.json())



        return response_200

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[RegistrationStatus]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    *,
    client: AuthenticatedClient | Client,

) -> Response[RegistrationStatus]:
    """ Whether self-registration is open

     Unauthenticated — a visitor deciding whether to show the login
    page's own "Register a new passkey instead" button has no session
    yet by definition. `open` is true only on a fresh instance with
    zero registered users; once any account exists, self-registration
    is closed and every registration after that requires a valid
    `inviteToken` (see POST /api/auth/register/begin).

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[RegistrationStatus]
     """


    kwargs = _get_kwargs(
        
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)

def sync(
    *,
    client: AuthenticatedClient | Client,

) -> RegistrationStatus | None:
    """ Whether self-registration is open

     Unauthenticated — a visitor deciding whether to show the login
    page's own "Register a new passkey instead" button has no session
    yet by definition. `open` is true only on a fresh instance with
    zero registered users; once any account exists, self-registration
    is closed and every registration after that requires a valid
    `inviteToken` (see POST /api/auth/register/begin).

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        RegistrationStatus
     """


    return sync_detailed(
        client=client,

    ).parsed

async def asyncio_detailed(
    *,
    client: AuthenticatedClient | Client,

) -> Response[RegistrationStatus]:
    """ Whether self-registration is open

     Unauthenticated — a visitor deciding whether to show the login
    page's own "Register a new passkey instead" button has no session
    yet by definition. `open` is true only on a fresh instance with
    zero registered users; once any account exists, self-registration
    is closed and every registration after that requires a valid
    `inviteToken` (see POST /api/auth/register/begin).

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[RegistrationStatus]
     """


    kwargs = _get_kwargs(
        
    )

    response = await client.get_async_httpx_client().request(
        **kwargs
    )

    return _build_response(client=client, response=response)

async def asyncio(
    *,
    client: AuthenticatedClient | Client,

) -> RegistrationStatus | None:
    """ Whether self-registration is open

     Unauthenticated — a visitor deciding whether to show the login
    page's own "Register a new passkey instead" button has no session
    yet by definition. `open` is true only on a fresh instance with
    zero registered users; once any account exists, self-registration
    is closed and every registration after that requires a valid
    `inviteToken` (see POST /api/auth/register/begin).

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        RegistrationStatus
     """


    return (await asyncio_detailed(
        client=client,

    )).parsed
